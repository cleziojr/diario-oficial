...
func TestPostAnalysis_Success(t *testing.T) {
	...
	err := c.PostAnalysis(context.Background(), "doc-id-123", "texto extraido")
	if err != nil {
...
func TestPostAnalysis_ServerError(t *testing.T) {
	...
	c := backend.New(srv.URL)
	err := c.PostAnalysis(context.Background(), "doc-id-123", "texto")
	if err == nil {
...
func TestPostAnalysis_ContextCancelled(t *testing.T) {
	...
	c := backend.New(srv.URL)
	err := c.PostAnalysis(ctx, "doc-id-123", "texto")
	if err == nil {
